...
const main = async () => {
    ...
    const [ageMin, ageMax] = age_range_days;

    console.log(`Filtering wallets with balance >= ${min_balance_sol} SOL and age between ${ageMin} and ${ageMax} days...`);
    ...
    const checkWallet = async (address) => {
        try {
            ...
            if (transactionCount > 0) {
                ...
                const blockTime = await withRetry(() => connection.getBlockTime(earliestSignature.slot));
                if (blockTime) {
                    firstTxTimestamp = new Date(blockTime * 1000);
                }
            }

            const ageDays = firstTxTimestamp
                ? Math.floor((Date.now() - firstTxTimestamp.getTime()) / (1000 * 60 * 60 * 24))
                : null;

            if (ageDays === null || ageDays < ageMin || ageDays > ageMax) {
                return null;
            }
    ...
    try {
        ...
        const outputFilePath = path.join(logs_dir_path, 'filteredWallets.txt');
